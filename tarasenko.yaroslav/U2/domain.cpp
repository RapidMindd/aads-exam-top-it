...
int tarasenko::findPersonIndex(const Database& database, size_t id)
{
...
bool tarasenko::hasPerson(const Database& database, size_t id)
{
  return findPersonIndex(database, id) >= 0;
}

bool tarasenko::hasPersonInfo(const Database& database, size_t id)
{
  const int index = findPersonIndex(database, id);
  return (index >= 0) && database.persons.data[index].hasInfo;
}

void tarasenko::ensurePerson(Database& database, size_t id)
{
...
void tarasenko::setPersonInfo(Database& database, size_t id, const std::string& info)
{
  const int index = findPersonIndex(database, id);
  if (index < 0) {
...
bool tarasenko::mergeAnonPerson(Database& database, size_t anonId, size_t id)
{
  ...
  for (size_t index = static_cast< size_t >(anonIndex) + 1;
